...
-   **Cloudinary**: Primary video host going forward. Server-side module at `src/lib/server/cloudinary.ts` handles fetching videos by section (hero, showcase, reels, testimonials) from Cloudinary folders (`righello/hero`, `righello/showcase`, `righello/reels`, `righello/testimonials`), building signed upload params, updating metadata, and deleting videos. Homepage loads via `+page.server.ts` with a 5-minute in-memory cache and automatic fallback to hardcoded YouTube/Firebase data (`src/lib/data/videos-fallback.ts`) when Cloudinary is empty. All video components support `cloudinaryUrl`, `cloudinaryPublicId`, and `thumbnailUrl` fields. CSP `media-src` includes `https://res.cloudinary.com` and `https://firebasestorage.googleapis.com` (for fallback).
-   **Admin Panel**: Password-protected panel at `/admin` (login) and `/admin/dashboard`. Authenticated via SHA-256 cookie (`righello_admin`) using `ADMIN_SECRET_KEY` secret. Dashboard shows section tabs (hero, showcase, reels, testimonials), video list with edit/delete, and direct-to-Cloudinary upload form with signed upload + progress bar. API routes: `/api/admin/upload` (GET signed params), `/api/admin/videos` (GET/PUT/DELETE).
